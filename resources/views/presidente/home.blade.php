@component('presidente.components.layouts.app')
    ...
    <nav id="sidebar">
        ...
        <div class="p-4">
            ...
            <div class="text-center">
                <a class="btn btn-danger btn-block mb-4 mr-2">EXPEDIENTE</a>
                {{-- <button type="button" class="btn btn-warning btn-block warning tribuna mb-4 mr-2">TRIBUNA</button> --}}
                <button type="button" class="btn btn-success btn-block warning ordemdodia mb-4 mr-2">ORDEM DO DIA</button>
            </div>
    ...
    <div id="content" class="p-4 p-md-5 pt-5">
        ...
        <div class="widget-content widget-content-area">
            ...
            <script>
                let checkboxes;
                let prepareVotingButton;
                let votingButton;
                let bulkVotingButton;
                let prepareBulkVotingButton;

                const updateButtons = () => {
                    const tableBody = document.getElementById('tableBody');
                    checkboxes = tableBody.querySelectorAll('.new-control-input');
                    const checkedCheckboxes = tableBody.querySelectorAll('.new-control-input:checked');
                    const selectedStatuses = Array.from(checkedCheckboxes).map(checkbox => checkbox.getAttribute(
                        'data-status'));
                    const uniqueStatuses = [...new Set(selectedStatuses)];
                ...
                window.onload = function() {
                    const tableBody = document.getElementById('tableBody');
                    checkboxes = tableBody.querySelectorAll('.new-control-input');
                    prepareVotingButton = document.getElementById('prepare-voting-button');
                    votingButton = document.getElementById('voting-button');
                    bulkVotingButton = document.getElementById('bulk-voting-button');
                    prepareBulkVotingButton = document.getElementById('prepare-bulk-voting-button');

                    checkboxes.forEach(checkbox => {
                        checkbox.addEventListener('change', updateButtons);
                        if (checkbox.checked && checkbox.getAttribute('data-status') === 'VOTACAO') {
                            updateButtons();
                        }
                    });
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <p class="modal-text">
                        <button class="btn btn-success mb-4 mr-3 btn-lg vote-button-bulk" data-vote="SIM">SIM</button>
                        <button class="btn btn-danger mb-4 mr-3 btn-lg vote-button-bulk" data-vote="NAO">NÃO</button>
                        <button class="btn btn-dark mb-4 btn-lg vote-button-bulk" data-vote="ABSTER">ABSTER</button>
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="modal fade" id="modal-votacao-em-bloco" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <div id="modal-success" class="alert alert-success" style="display: none;">
                        <strong>Voto enviado com sucesso!</strong>
                    </div>
    ...
    {{-- <script>
        $(document).ready(function() {
            ...
            function triggerDocumentosSelecionadosUpdatedEvent() {
                var documentosSelecionadosArray = Object.values(documentosSelecionados);
                $(document).trigger('documentosSelecionadosUpdated', [documentosSelecionadosArray]);
            }

            function checkDocumentosStatus() {
                $(".todochkbox").each(function() {
                    var documentoTitle = $(this).closest("tr").find("td:nth-child(2)").text();
                    var nrSequence = $(this).attr("data-nrsequence");
                    var status = $(this).attr("data-status");

                    if ($(this).is(":checked") && status === "VOTACAO") {
                        documentosSelecionados[nrSequence] = documentoTitle;
                    }
                });

                // Disparar o evento para notificar a atualização dos documentos selecionados
                triggerDocumentosSelecionadosUpdatedEvent();
            }

            checkDocumentosStatus();

            $(document).on("change", ".todochkbox", function() {
                var documentoTitle = $(this).closest("tr").find("td:nth-child(2)").text();
                var nrSequence = $(this).attr("data-nrsequence");
                var status = $(this).attr("data-status");
                ...
                if ($(this).is(":checked") && status === "VOTACAO") {
                    ...
                    delete documentosSelecionados[nrSequence];
                }
            ...
            $("#modal-votacao-em-bloco").on("show.bs.modal", function() {
                $("#document-list").empty();
                var documentosSelecionadosArray = Object.values(documentosSelecionados);
                documentosSelecionadosArray.forEach(function(documento) {
                    $("#document-list").append("<li>" + documento + "</li>");
                });
            });

            $(document).on('documentosSelecionadosUpdated', function(event, documentosSelecionadosArray) {
                $("#document-list").empty();
                documentosSelecionadosArray.forEach(function(documento) {
                    $("#document-list").append("<li>" + documento + "</li>");
                });
            ...
            $(".vote-button-bulk").click(function() {
                var voto = $(this).attr("data-vote");

                if (Object.keys(documentosSelecionados).length > 0) {
                    var nrSequenceArray = Object.keys(documentosSelecionados);
                    var nrSequence = $('meta[name="nr-sequence"]').attr("content");

                    var data = {
                        usuarioVoto: {
                            nrSequence: parseInt(nrSequence)
                        },
                        voto: voto,
                        documentos: nrSequenceArray
                    };
                    ...
                    $("#modal-votacao-em-bloco").modal("hide");
                } else {
                    alert("Selecione pelo menos um documento para votar em bloco.");
                }
            });
    ...
    {{-- <script>
        $(document).ready(function() {
            ...
            $(".todochkbox:checked").each(function() {
                ...
                // Trigger the event to notify selected documents updated
                triggerDocumentosSelecionadosUpdatedEvent();
            });
            ...
            $(".vote-button-bulk").click(function() {
                ...
                if (Object.keys(documentosSelecionados).length > 0) {
                    ...
                    var data = {
                        ...
                        voto: voto,
                        documentos: nrSequenceArray.map(Number)
                    };
        ...
        $(document).ready(function() {
            ...
            $(".vote-button-bulk").click(function() {
                ...
                if (Object.keys(documentosSelecionados).length > 0) {
                    ...
                    console.log("Dados enviados para a API: ", data); // Console log adicionado aqui
        ...
        $(document).ready(function() {
            ...
            $(".vote-button-bulk").click(function() {
                ...
                if (Object.keys(documentosSelecionados).length > 0) {
                    ...
                    var documentos = nrSequenceArray.map(Number);
                    ...
                    $.ajax({
                        ...
                        error: function(xhr, status, error) {
                            ...
                            // Exibir a resposta da API no modal
                            $("#modal-error-message").text(responseJSON.message);
                            $("#modal-error").show();
                        },
                        complete: function() {
                            // Restaurar o estado inicial do modal
                            $(".vote-button-bulk").prop("disabled", false);
                        }
                    });
                ...
                if (Object.keys(documentosSelecionados).length > 0) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            // Exibir a mensagem de "Voto enviado com sucesso" no modal
                            $("#modal-success-message").text("Voto enviado com sucesso!");
                            $("#modal-success").show();
                        },
                        error: function(xhr, status, error) {
                            var responseJSON = xhr.responseJSON;
